...
for x, sw, rn, rv, nm in ((139.7, "SW2", "R7", "20k", "LEVER A"),
    ...
    place("SW_Lever_MT1", sw, nm, x, 118.11, 90)
    place("R", rn, rv, x, 130.81)
    wire(x, AY, x, 114.3); wire(x, 121.92, x, 127.0)
    wire(x, 134.62, x, 139.7); label("GND", x, 139.7, 270, "left bottom")
junc(165.1, AY)
...
# --- Connector --------------------------------------------------------------
place("Conn_JST_XH_6", "J1", "JST-XH 6", 261.62, 80.01)
for i, n in enumerate(NM):
    y = 78.74 + 2.54 * i
    wire(269.24, y, 279.4, y)
    label(n, 279.4, y, 0, "left bottom")
